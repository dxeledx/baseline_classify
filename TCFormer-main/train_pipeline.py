...
def train_and_test(config):
    ...
    for subject_id in subject_ids:
        ...
        # 检查是否有GPU可用
        import torch
        use_gpu = torch.cuda.is_available() and config.get("gpu_id", 0) >= 0
        
        trainer = Trainer(
            max_epochs=config["max_epochs"],
            devices=1 if not use_gpu else ([config.get("gpu_id", 0)] if config.get("gpu_id", 0) != -1 else -1),
            num_sanity_val_steps=0,
            accelerator="gpu" if use_gpu else "cpu",
            strategy="auto" if config.get("gpu_id", 0) != -1 else DDPStrategy(find_unused_parameters=True), 
            logger=False,
