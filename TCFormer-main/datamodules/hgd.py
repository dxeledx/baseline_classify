...
class HighGamma(BaseDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        X = ds.load_data()._data if hasattr(ds, 'load_data') else ds.windows.load_data()._data
        y = np.array(train_dataset.datasets[0].y)
        ds_test = test_dataset.datasets[0]
        X_test = ds_test.load_data()._data if hasattr(ds_test, 'load_data') else ds_test.windows.load_data()._data
...
class HighGammaLOSO(HighGamma):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        # load the test data (compatible with braindecode 0.8.x)
        ds_test = test_dataset.datasets[0]
        X_test = ds_test.load_data()._data if hasattr(ds_test, 'load_data') else ds_test.windows.load_data()._data
        y_test = np.array(test_dataset.datasets[0].y)
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = (ds.load_data()._data if hasattr(ds, 'load_data') else ds.windows.load_data()._data)[idx]
