...
class Graph_CSPNet_Basic(nn.Module):
    ...
    def __init__(self, channel_num, P, mlp, dataset = 'KU'):
        ...
        self._mlp       = mlp
        self.channel_in = channel_num
        # Register P as a buffer so it moves with the model to GPU/CPU
    ...
    def _make_Graph_BiMap_block(self, layer_num):
        ...
        layers = []
        # Create identity matrix on the same device as P
        _I     = th.eye(self.P.shape[0], dtype=th.double, device=self.P.device)
