...
try:  # numpy 2.x exposes explicit dtype classes
    from numpy.dtypes import Float64DType as _NPFloat64DType
except ImportError:  # fallback for numpy 1.x
    _NPFloat64DType = type(np.dtype("float64"))
...
def train_and_test(config):
    ...
    for subject_id in subject_ids:
        ...
        checkpoint_callback = ModelCheckpoint(
            ...
            mode="min",
            save_top_k=1,
            filename=f"subject{subject_id}-best",
...
def parse_arguments():
    ...
    parser.add_argument("--model", type=str, default="tcformer", choices=["tcformer"],
        help="仅保留 TCFormer 训练管线，如需其他模型请使用原仓库"
    )        
